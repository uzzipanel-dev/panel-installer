...
# Download URLs
export PANEL_DL_URL="https://github.com/uzzipanel-dev/panel/releases/latest/download/panel.tar.gz"
export WINGS_DL_URL="https://github.com/uzzipanel-dev/wings/releases/download/wings/wings.tar.gz"
export GIT_REPO_URL="https://raw.githubusercontent.com/uzzi-installer/uzzi-installer/Production"
...
panel_dl() {
  output "Downloading uzzi panel files .. "
  mkdir -p /var/www/uzzi/storage/framework/cache/data/{9c,9c/a8,8a,8a/69}
  cd /var/www/uzzi || exit
  ...
  success "Downloaded Uzzi Panel files!"
}
...
configure_env() {
  ...
  for file in /var/www/uzzi/database/Seeders/eggs/*/*.json; do
  ...
  # cp /var/www/uzzi/.env /etc/uzzi/.env
  ...
  php artisan p:node:make \
    ...
    --daemonSFTPPort=2022 \
    --daemonBase="/var/lib/uzzi/volumes"

  # Fetch wings configuration
  mkdir -p /etc/uzzi
  echo "$(php artisan p:node:configuration 1)" > /etc/uzzi/config.yml
...
insert_cronjob() {
  ...
  crontab -l | {
    cat
    output "* * * * php /var/www/uzzi/artisan schedule:run >> /dev/null 2>&1"
  } | crontab -
...
php_fpm_conf() {
  curl -o /etc/php-fpm.d/www-uzzi.conf "$GIT_REPO_URL"/configs/www-uzzi.conf
...
configure_nginx() {
  ...
  ubuntu | debian)
    ...
    ;;
  rocky | almalinux)
    PHP_SOCKET="/var/run/php-fpm/uzzi.sock"
  ...
  curl -o "$CONFIG_PATH_AVAIL"/uzzi.conf "$GIT_REPO_URL"/configs/nginx.conf
  sed -i -e "s@<domain>@${IP_ADDRESS}@g" "$CONFIG_PATH_AVAIL"/uzzi.conf
  sed -i -e "s@<php_socket>@${PHP_SOCKET}@g" "$CONFIG_PATH_AVAIL"/uzzi.conf

  case "$OS" in
  ubuntu | debian)
    ln -sf "$CONFIG_PATH_AVAIL"/uzzi.conf "$CONFIG_PATH_ENABL"/uzzi.conf
    ;;
  esac
...
wings_dl() {
  ...
  success "Uzzi Wings downloaded successfully"
}
...
# --------------- Execute functions --------------- #
output "Starting Uzzi Panel installation.. this might take a while!"
panel_deps
install_composer
panel_dl
install_composer_deps
create_db_user "uzzi" "$MYSQL_PASSWORD"
create_db "panel" "uzzi"
configure_env
insert_cronjob
pteroq_systemd
configure_nginx
