...
php_fpm_conf() {
  curl -o /etc/php-fpm.d/www-uzzi.conf "$GITHUB_URL"/configs/www-uzzi.conf
...
configure_nginx() {
  ...
  curl -o "$CONFIG_PATH_AVAIL"/uzzi.conf "$GITHUB_URL"/configs/$DL_FILE

  sed -i -e "s@<domain>@${FQDN}@g" "$CONFIG_PATH_AVAIL"/uzzi.conf
