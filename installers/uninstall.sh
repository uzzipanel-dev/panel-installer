...
rm_panel_files() {
  ...
  rm -rf /var/www/uzzi /usr/local/bin/composer
  [ "$OS" != "centos" ] && unlink /etc/nginx/sites-enabled/uzzi.conf
  [ "$OS" != "centos" ] && rm -f /etc/nginx/sites-available/uzzi.conf
  [ "$OS" != "centos" ] && ln -s /etc/nginx/sites-available/default /etc/nginx/sites-enabled/default
  [ "$OS" == "centos" ] && rm -f /etc/nginx/conf.d/uzzi.conf
...
rm_wings_files() {
  ...
  rm -rf /etc/uzzi /usr/local/bin/wings /var/lib/uzzi
...
rm_services() {
  ...
  centos)
    ...
    rm -rf /etc/php-fpm.d/www-uzzi.conf
    ;;
  esac
...
rm_cron() {
  ...
  crontab -l | grep -vF "* * * * * php /var/www/uzzi/artisan schedule:run >> /dev/null 2>&1" | crontab -
...
rm_database() {
  ...
  if [[ "$valid_db" == *"panel"* ]]; then
    echo -n "* Database called panel has been detected. Is it the Uzzi database? (y/N): "
    read -r is_panel
    if [[ "$is_panel" =~ [Yy] ]]; then
      DATABASE=panel
    else
  ...
  if [[ "$valid_users" == *"uzzi"* ]]; then
    echo -n "* User called uzzi has been detected. Is it the uzzi user? (y/N): "
    read -r is_user
    if [[ "$is_user" =~ [Yy] ]]; then
      DB_USER=uzzi
    else
