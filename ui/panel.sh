...
main() {
  ...
  if [ -d "/var/www/uzzi" ]; then
    warning "The script has detected that you already have Uzzi panel on your system! You cannot run the script multiple times, it will fail!"
  ...
  MYSQL_USER="-"
  while [[ "$MYSQL_USER" == *"-"* ]]; do
    required_input MYSQL_USER "Database username (uzzi): " "" "uzzi"
  ...
  email_input email "Provide the email address that will be used to configure Let's Encrypt and Uzzi: " "Email cannot be empty or invalid"
...
summary() {
  print_brake 62
  output "Uzzi panel $UZZI_PANEL_VERSION with nginx on $OS"
