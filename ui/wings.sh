...
export MYSQL_DBHOST_USER="uzziuser"
...
main() {
  ...
  if [ -d "/etc/uzzi" ]; then
    warning "The script has detected that you already have Uzzi wings on your system! You cannot run the script multiple times, it will fail!"
  ...
  echo "* official documentation: $(hyperlink 'https://hub.uzzinetworkhosting.ro/docs/wings/install#configure')"
  echo "* "
  ...
  if [ "$CONFIGURE_DBHOST" == true ]; then
    ...
    while [[ "$MYSQL_DBHOST_USER" == *"-"* ]]; do
      required_input MYSQL_DBHOST_USER "Database host username (uzziuser): " "" "uzziuser"
...
function goodbye {
  ...
  echo "* Please refer to the official guide, $(hyperlink 'https://hub.uzzinetworkhosting.ro/docs/wings/install#configure')"
  echo "* "
  echo "* You can either copy the configuration file from the panel manually to /etc/uzzi/config.yml"
